...
import {
  ...
  EVENT_MANAGER_CATEGORY_UNSELECTED, EVENT_MANAGER_COMPETITION_IMAGE_LOADED, EVENT_MANAGER_COMPETITION_IMAGE_REMOVED,
...
export function competitionStateReducer(st: CompetitionState = initialCompetitionState, action) {
  ...
  return produce(st, state => {
    switch (action.type) {
      ...
      case EVENT_MANAGER_COMPETITION_IMAGE_REMOVED: {
        state.selectedEventInfoImage = null;
        break;
      }
      ...
      case EventType.FIGHTS_START_TIME_UPDATED: {
        ...
        if (newFights) {
          ...
          state.fights
            = fightEntityAdapter.updateMany(updates, state.fights);
      ...
      case DASHBOARD_FIGHT_UNSELECTED: {
        state.fights.selectedFightId = null;
        break;
      }
...
export const dashboardGetSelectedPeriodMatsFightsCollection = createSelector(getSelectedEventState, state => (state && state.fights) || fightsInitialState);
