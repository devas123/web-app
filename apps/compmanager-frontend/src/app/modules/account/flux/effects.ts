...
export class AccountEffects {
  ...
  userAuthorized = createEffect(() => this.actions$.pipe(ofType(USER_AUTHORIZED), switchMap((action: CommonAction) => {
  ...
  authorizeUser: Observable<Action> = createEffect(() => this.actions$.pipe(ofType(AUTHORIZE_USER), mergeMap((action: any) => {
  ...
  authorizeToken: Observable<Action> = createEffect(() => this.actions$.pipe(ofType(AUTHORIZE_TOKEN),
  ...
  changeAvatar = createEffect(() => this.actions$.pipe(ofType(CHANGE_AVATAR), switchMap((action: CommonAction) => this.accountService.changeAvatar(action.payload.blobBase64)), catchError(err => of(accountError(err)))), {dispatch: false});
}
